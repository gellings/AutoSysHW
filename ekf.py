...
class EKF:
    ...
    def propogate(self, u, dt):
        ...
        self.x += np.array([[v/w*(-np.sin(theta) + np.sin(theta + w*dt))],
                            ...
                            [w*dt]])
        R = np.diag([[0.001, 0.001, 0.00001]])

        # print A.shape, self.P.shape, G.shape, Qu.shape
        self.P = A.dot(self.P).dot(A.T) + G.dot(Qu).dot(G.T) #+ R
    ...
    def state_elps(self):
        ...
        return 4*np.sqrt(vals[0]), 4*np.sqrt(vals[1]), np.arctan2(vecs[1,0],vecs[0,0])